...
import type { Sticker, StickerIndex, Vec3 } from './model';
import { FACE_ORDER } from './moves';
import type { Face } from './moves';

export function createFaceGrid<T>(value: T): T[][] {
  return Array.from({ length: 3 }, () => [value, value, value]);
}

export function toFaceCoordinates(face: Face, position: Vec3): [StickerIndex, StickerIndex] {
  switch (face) {
    case 'U':
...
export function toVisibleFace(normal: Vec3): Face {
...
export function getStickerRotationQuarterTurns(sticker: Pick<Sticker, 'up'>, visibleFace: Face): 0 | 1 | 2 | 3 {
