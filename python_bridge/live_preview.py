...
<html lang="en">
  <head>
    ...
    <title>Rubik Experiment Live Viewer</title>
    <style>
      :root {
        color-scheme: light;
        --bg: #f4efe8;
  ...
  <body>
    <div class="shell">
      <div class="hero">
        <h1 id="viewer-title">Rubik Experiment Live Viewer</h1>
        <p class="subtitle" id="viewer-subtitle">Live status on top, notebook-style preview history below.</p>
      </div>
      ...
      <div class="status">
        ...
        <div class="panel metric">
          <div class="metric-label" id="view-count-label">View Count</div>
      ...
      <div class="section-head">
        ...
        <div class="helper" id="timeline-helper">Each preview row is appended below the previous one, like the notebook.</div>
      </div>
    ...
    <script>
      ...
      async function refresh() {
        try {
          ...
          const viewerTitle = status.viewer_title || "Rubik Experiment Live Viewer";
          const viewerSubtitle = status.viewer_subtitle || "Live status on top, notebook-style preview history below.";
          document.title = viewerTitle;
          setText("viewer-title", viewerTitle);
          setText("viewer-subtitle", viewerSubtitle);
          setText("view-count-label", status.view_count_label || "View Count");
          ...
          setText("timeline-helper", status.timeline_helper || "Each preview row is appended below the previous one, like the notebook.");
...
def optional_viewer_server(
    ...
    *,
    ...
    write_live_viewer(viewer_root / viewer_path)
    if not enabled:
        yield None
        return
    ...
    handler = partial(QuietSimpleHTTPRequestHandler, directory=str(viewer_root))
    server = ThreadingHTTPServer((host, port), handler)
    thread = Thread(target=server.serve_forever, daemon=True)
    thread.start()

    viewer_url = f"http://{host}:{port}/{viewer_path}"
